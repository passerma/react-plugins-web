...
const PluginClass: React.FC<propsType> = (props) => {
    const [current, setcurrent] = useState('all')
    const serach = useRef('')

    const _handleClick = (e: MenuInfo) => {
        let path = `${e.keyPath[1] || 'more'}_${e.keyPath[0]}`
        props.history.push(`/plugins/${path}?page=1&size=9&search=${serach.current}`)
    }
    ...
    useEffect(() => {
        ...
        serach.current = urlQuery.search || ''
        setcurrent(typeArr[1] || 'allTime')
    }, [props.match.params])

    return (
        <div className="plugins-pluginClass">
            <Menu className="plugins-pluginClass-menu" onClick={_handleClick} selectedKeys={[current]} mode="horizontal"
                theme="light">
